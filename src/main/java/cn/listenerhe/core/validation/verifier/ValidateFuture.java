...
import cn.listenerhe.core.annotation.validation.Future;
 ...
 * @Description: 验证将来的日期
 */
public class ValidateFuture extends ValidateAbstract<Future> {


    @Override
    protected boolean supportsParameter(Parameter parameter, Class<Future> future) {
        return parameter != null && parameter.getAnnotation(Future.class) != null;
    }
        ...
        if(o != null){
            if(o instanceof Date){
                ...
                switch (annotation.type()){
                    ...
                    default:
                        throw new ValidateException("错误的配置：hehh.core.validation.annotation.Future type not "+annotation.type());
                }
                if(!temp){
                    String msg = annotation.msg();
                    if(StrKit.isBlank(msg)){
                        msg = parameter.getName()+"必须为将来的日期";
                    }
                    return new ErrorResult(Code.PARAM_ERROR,msg);
                }
            }else{
                 throw new ValidateException("错误的配置：hehh.core.validation.annotation.Future expect java.util.Date not "+o.getClass());
            }
        }
